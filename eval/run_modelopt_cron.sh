...
#
#   */30 * * * * /home/autotiny/Desktop/sparkinfer/eval/run_modelopt_cron.sh >> /tmp/sparkinfer_modelopt_bot.log 2>&1
#
...
flock -w 120 9 || { echo "[$(date -u +%FT%TZ)] lock held 120s+ — previous bot run still active, skipping modelopt tick"; exit 0; }
...
if gpu_ready; then
  ...
  python3 eval/pr_modelopt_bot.py "${BOT_ARGS[@]}"
else
  echo "[$TS] sparkinfer Qwen3.8-27B bot — pinned GPU $GPU_LABEL down — labels only"
  python3 eval/pr_modelopt_bot.py "${BOT_ARGS[@]}" --labels-only
fi
