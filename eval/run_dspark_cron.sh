...
#
#   Hourly, taking over the :00 slot run_modelopt_cron.sh held. DSpark decode@128 replaces
#   ModelOpt long-context as the scored dimension (2026-08-18, explicit user decision), so the two
#   are NOT meant to run together — both drive the one pinned GPU, and both want the same lock.
#
#   A round is cheaper than the ModelOpt bot's was, because it benches ONE context and skips the
#   Qwen3.6/Qwen3.8 sweep guards and the prefill parity check (see the bot's module docstring for
#   why none of those can move this dimension). Measured on the pinned RTX 5090 (2026-08-18), per
#   ref: ~25s model load + ~1.5s AR + ~2.5s DSpark + ~6s score dump, plus build. So roughly 1 min
#   per ref, i.e. a round is ~2 min with one PR.
#
...
#   • GPU up → full DSpark decode@128 eval + differential-accuracy gate + losslessness gate;
